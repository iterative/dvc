...
class HashFile:
    ...
    def check(self, odb):
        actual = get_hash(self.path_info, self.fs, odb.fs.hash_name)
...
class File(HashFile):
    ...
    def stage(cls, odb, path_info, fs, **kwargs):
        hash_info = get_hash(path_info, fs, odb.fs.hash_name, **kwargs)
...
class Tree(HashFile):
    ...
    def save_dir_info(cls, odb, dir_info, hash_info=None):
        if hash_info and hash_info.name == odb.fs.hash_name:
            try:
                odb.check(hash_info)
        ...
        hash_info = get_hash(tmp_info, odb.fs, odb.fs.hash_name)
