...
def _stage_file(fs_path, fs, name, odb=None, upload_odb=None, dry_run=False):
    ...
    if dry_run:
        ...
        odb.add(fs_path, fs, hash_info, move=False)
...
def _build_tree(fs_path, fs, name, **kwargs):
    ...
    for file_fs_path, meta, obj in _iter_objects(fs_path, fs, name, **kwargs):
        ...
        rel_fs_path = fs.path.relpath(file_fs_path, fs_path)
        rel_fs_path_parts = fs.path.parts(rel_fs_path)
        tree.add(rel_fs_path_parts, meta, obj.hash_info)
...
def _stage_tree(fs_path, fs, fs_info, name, odb=None, **kwargs):
    ...
    odb.add(tree.fs_path, tree.fs, tree.hash_info, move=True)
...
def _make_staging_url(
    fs: "BaseFileSystem", odb: "ObjectDB", maybe_fs_path: "Optional[str]"
):
    from dvc.scheme import Schemes

    url = f"{Schemes.MEMORY}://{_STAGING_MEMFS_PATH}"

    if maybe_fs_path is not None:
        fs_path = maybe_fs_path
        if odb.fs.scheme == Schemes.LOCAL:
            fs_path = odb.fs.path.abspath(fs_path)
