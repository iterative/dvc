...
class Output:
    ...
    def __init__(
        ...
        if self.use_cache and self.odb is None:
            raise RemoteCacheRequiredError(self.fs_path)
    ...
    def _parse_path(self, fs, fs_path):
        ...
        if parsed.scheme != "remote":
            ...
            if self.stage and not fs.path.isabs(fs_path):
    ...
    def commit(self, filter_info=None):
        ...
        if self.use_cache:
            granular = (
                ...
                and filter_info
                and not self.fs.path.eq(filter_info, self.fs_path)
            )
            if granular:
    ...
    def dumpd(self):
        ...
        if self.is_in_repo:
            path = self.fs.path.as_posix(
                self.fs.path.relpath(self.fs_path, self.stage.wdir)
            )
        else:
            path = self.def_path
    ...
    def verify_metric(self):
        ...
        if os.path.isdir(self.fs_path):
            ...
            logger.debug(msg, self.fs_path, name)
            return
    ...
    def get_obj(self, filter_info=None, **kwargs):
        ...
        if filter_info and not fs_path.eq(filter_info, self.fs_path):
    ...
    def checkout(
        ...
        **kwargs,
        ...
        obj = self.get_obj(filter_info=filter_info)
        if not obj and (
            filter_info and not self.fs.path.eq(filter_info, self.fs_path)
        ):
    ...
    def get_files_number(self, filter_info=None):
        ...
        if not filter_info or self.fs.path.eq(filter_info, self.fs_path):
    ...
    def _collect_used_dir_cache(
        ...
        obj = self.get_obj()
        if filter_info and not self.fs.path.eq(filter_info, self.fs_path):
            prefix = self.fs.path.parts(
                self.fs.path.relpath(filter_info, self.fs_path)
            )
