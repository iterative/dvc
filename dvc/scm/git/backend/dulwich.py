...
class DulwichBackend(BaseGitBackend):  # pylint:disable=abstract-method
    ...
    def add(self, paths: Union[str, Iterable[str]], update=False):
        ...
        for path in paths:
            if not os.path.isabs(path) and self._submodules:
                ...
                fs_path = manager.relpath(path, self.root_dir)
                for sm_path in self._submodules.values():
                    if manager.isin(fs_path, sm_path):
                        path = os.path.join(
                            self.root_dir,
                            manager.relpath(fs_path, sm_path),
                        )
                        break
    ...
    def is_tracked(self, path: str) -> bool:
        rel = manager.as_posix(manager.relpath(path, self.root_dir)).encode()
        rel_dir = rel + b"/"
