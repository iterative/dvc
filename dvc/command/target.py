...
class CmdTarget(CmdBase):
    ...
    def run(self):
        with DvcLock(self.is_locker, self.git):
            return self.change_target(self.parsed_args.target, self.parsed_args.unset)

    def change_target(self, target, unset):
        if target and unset:
            Logger.error('Unable to set target {} and use --unset in a single command'.format(target, unset))
            return 1
        ...
        if unset:
            if not os.path.exists(target_conf_file_path.relative):
            ...
            if open(target_conf_file_path.relative).read() == '':
                return 0
