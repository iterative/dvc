...
class Experiments:
    ...
    def get_running_exps(self) -> Dict[str, Any]:
        """Return info for running experiments."""
        result = {}
        infofile = self.workspace_queue.get_infofile_path("workspace")
        result.update(self._get_running_exp("workspace", infofile))
        for entry in self.celery_queue.iter_active():
            infofile = self.celery_queue.get_infofile_path(entry.stash_rev)
            result.update(self._get_running_exp(entry.stash_rev, infofile))
        return result

    def _get_running_exp(self, rev: str, infofile: str) -> Dict[str, Any]:
