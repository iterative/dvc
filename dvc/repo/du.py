...
def du(url, path=None, rev=None, summarize=False, max_depth=-1, include_files=False):
    ...
    with Repo.open(url, rev=rev, subrepos=True, uninitialized=True) as repo:
        ...
        path_size = {}
        try:
            path_size[fs_path] = _get_path_size(fs, fs_path, max_depth, path_size, include_files=include_files)
        except FileNotFoundError:
        ...
        for path, size in path_size.items():
            ...
            if is_dvc_file(path):
                ...
                fixed_path = ''.join(fixed_path.split('.')[:-1]) + " [*] DVC Tracked Data"
            if size:
                ret_list.append({
                    "path": fixed_path,
                    "size": size
