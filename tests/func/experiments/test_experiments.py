...
def test_list(tmp_dir, scm, dvc, exp_stage):
    ...
    assert {key: set(val) for key, val in exp_list.items()} == {
        baseline_a[:7]: {(ref_info_a.name, exp_a[:7]), (ref_info_b.name, exp_b[:7])},
        "branch": {(ref_info_c.name, exp_c[:7])},
    }
