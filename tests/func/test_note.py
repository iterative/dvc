...
def test_note_set_fails_when_key_not_given(dvc):
    with pytest.raises(DvcException):
        dvc.note("set", ["alpha.txt"], None, "green")


def test_note_set_fails_when_value_not_given(dvc):
    with pytest.raises(DvcException):
...
def test_note_set_fails_when_single_file_or_dvc_doesnt_exist(tmp_dir, scm, dvc):
    note_setup(tmp_dir)
...
def test_note_set_fails_without_change_when_any_file_doesnt_exist(tmp_dir, scm, dvc):
    note_setup(tmp_dir)
...
def test_note_find_none_when_none_added(tmp_dir, scm, dvc):
    note_setup(tmp_dir)
    assert dvc.note("find", ["alpha.txt"], "color") == []


def test_note_find_one_when_one_added(tmp_dir, scm, dvc):
    note_setup(tmp_dir)

    dvc.note("set", ["alpha.txt"], "color", "green")

    assert dvc.note("find", ["alpha.txt"], "color") == [["alpha.txt", "color", "green"]]


def test_note_find_one_when_many_added_to_one_file(tmp_dir, scm, dvc):
    note_setup(tmp_dir)
...
def test_note_find_none_when_looking_for_another_key(tmp_dir, scm, dvc):
    note_setup(tmp_dir)
...
def test_note_find_some_when_multiple_keys_in_multiple_files(tmp_dir, scm, dvc):
    note_setup(tmp_dir)
    ...
    assert dvc.note("find", ["alpha.txt", "beta.txt", "gamma/delta.txt"], "size") == [
        ["beta.txt", "size", "small"],
        ["gamma/delta.txt", "size", "medium"]
    ]


def test_note_find_fails_when_file_doesnt_exist(tmp_dir, scm, dvc):
    note_setup(tmp_dir)
...
def test_note_list_fails_when_file_doesnt_exist(tmp_dir, scm, dvc):
    note_setup(tmp_dir)
...
def test_note_list_when_one_added(tmp_dir, scm, dvc):
    note_setup(tmp_dir)
...
def test_note_list_when_many_added_to_one_file(tmp_dir, scm, dvc):
    note_setup(tmp_dir)

    dvc.note("set", ["alpha.txt"], "color", "green")
    dvc.note("set", ["alpha.txt"], "size", "medium")

    assert dvc.note("list", ["alpha.txt"]) == [["alpha.txt", ["color", "size"]]]


def test_note_list_when_many_added_to_many_files(tmp_dir, scm, dvc):
    note_setup(tmp_dir)
    paths = ["alpha.txt", "beta.txt", "gamma/delta.txt"]
    ...
    assert dvc.note("list", paths) == [["alpha.txt", ["color", "size"]],
                                       ["beta.txt", ["color"]],
                                       ["gamma/delta.txt", ["flavor"]]]


def test_note_remove_fails_when_key_not_given(dvc):
    with pytest.raises(DvcException):
        dvc.note("remove", ["alpha.txt"], None, "green")


def test_note_remove_fails_when_file_doesnt_exist(tmp_dir, scm, dvc):
    note_setup(tmp_dir)
