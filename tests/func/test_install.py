...
def test_create_hooks(tmp_dir, scm, dvc):
    scm.install()
    ...
    for fname, command in hooks_with_commands:
        hook_path = tmp_dir / ".git" / "hooks" / fname
        assert hook_path.is_file()
        assert command in hook_path.read_text()


def test_fail_if_hook_exists(tmp_dir, scm):
    (tmp_dir / ".git" / "hooks" / "post-checkout").write_text("hook content")

    with pytest.raises(GitHookAlreadyExistsError):
        scm.install()


def test_post_checkout(tmp_dir, scm, dvc):
    scm.install()
...
def test_pre_push_hook(tmp_dir, scm, dvc, tmp_path_factory):
    scm.install()
