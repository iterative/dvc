...
def test_run_duplicate_exp(dvc, scm, mocker):
    mocker.patch("dvc.repo.experiments.ls.ls", return_value={"rev": ["name"]})

    cli_args = parse_args(["experiments", "run", "-n", "name"])
    with pytest.raises(InvalidArgumentError):
        CmdExperimentsRun(cli_args).run()
